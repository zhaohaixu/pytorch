...
public:  
    ...
    {  
        ...
        this->localSizePadding = (localSize + 64 - 1) / 64 * 64;
        this->dataSize = dataSize;
    ...
    {  
        if (GetBlockIdx() * this->localSizePadding < this->dataSize) {
            ...
            for (; i < dataSizePerThread; i += blocksToProcess){
                ...
                if (blocksToProcess % 64 != 0) {
                    blocksToProcess = blocksToProcess / 64 * 64;
                }
                if (blocksToProcess < 64)
                    break;
                // if (GetBlockIdx() <= 23) {
                //     AscendC::printf("line 427: [%d, %d]", i, blocksToProcess / sizeof(uint16_t));
                // }
                // if (this->dataSize == 42 || this->dataSize == 43) {
                // if (this->dataSize != 42 && this->dataSize != 43) {
                //     AscendC::printf("line 427: [%x]\n", inputGlobal_uint8(0));
                // }
...
class KernelChaCha20Generation_loop {  
public:  
    __aicore__ inline KernelChaCha20Generation_loop() {}  
    ...
    __aicore__ inline void Init(__gm__ uint8_t* state, __gm__ uint8_t* output, uint32_t dataSize)  
    {  
        stateGlobal.SetGlobalBuffer((__gm__ uint32_t*)state);  
        outputGlobal.SetGlobalBuffer((__gm__ uint8_t*)output + GetBlockIdx() * (dataSize / GetBlockNum()));
        this->dataSize = dataSize;  
        this->totalBlocks = (dataSize + BLOCK_SIZE - 1) / BLOCK_SIZE;  
        this->localBlocks = (dataSize + BLOCK_SIZE - 1) / BLOCK_SIZE / GetBlockNum();  
    ...
    __aicore__ inline void Process()
    {  
        uint32_t threadId = GetBlockIdx();  
        uint32_t threadNum = GetBlockNum();  
        uint32_t dataSizePerThread = dataSize / threadNum;
        uint32_t blocksToProcess = min(static_cast<uint32_t>(DOUBLE_BUFFER_SIZE), dataSizePerThread); // 每次处理多少字节

        LocalTensor<uint32_t> stateLocal = stateQueue.AllocTensor<uint32_t>();  
...
private:  
    ...
    __aicore__ inline void CopyIn(uint32_t startBlock, uint32_t blocksToProcess)  
    {  
        LocalTensor<uint32_t> stateLocal = stateQueue.DeQue<uint32_t>();  
        stateQueue.EnQue(stateLocal);  
    }  
    ...
    __aicore__ inline void ComputeVectorized(uint32_t startBlock, uint32_t blocksToProcess)  
    {   
        LocalTensor<uint8_t> outputLocal = outputQueue.AllocTensor<uint8_t>();
    ...
    // 优化的XOR操作 - 直接使用指针访问
    __aicore__ inline void fastXOR(uint32_t* keystream, 
                                  LocalTensor<uint8_t>& output, uint32_t blockOffset)
    {
        // 按64字节块处理，4字节对齐
        for (int i = 0; i < 16; i++) {
            uint32_t wordOffset = blockOffset + i * 4;
    ...
    __aicore__ inline void CopyOut(uint32_t startBlock, uint32_t blocksToProcess)
    {
        LocalTensor<uint8_t> outputLocal = outputQueue.DeQue<uint8_t>();
        
        uint32_t outputOffset = startBlock * BLOCK_SIZE;
        uint32_t outputSize = blocksToProcess * BLOCK_SIZE;
        
        if (outputOffset + outputSize > localBlocks * BLOCK_SIZE) {
            outputSize = dataSize - outputOffset;
        }
        
        DataCopy(outputGlobal[outputOffset], outputLocal, outputSize);
        outputQueue.FreeTensor(outputLocal);
    }
    
private:
    TPipe pipe;
    TQue<TPosition::VECIN, 1> stateQueue;
    TQue<TPosition::VECOUT, 2> outputQueue;
    TQue<TPosition::VECIN, 2> workingQueue;

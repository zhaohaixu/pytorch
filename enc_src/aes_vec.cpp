...
public:
    ...
    // LocalTensor<uint32_t> scatterIdx0U32;
    // LocalTensor<uint32_t> scatterIdx1U32;
    // LocalTensor<uint32_t> scatterIdx2U32;
    // LocalTensor<uint32_t> scatterIdx3U32;
...
private:
    ...
    __aicore__ inline void Compute(uint32_t startBlock, uint32_t blocksToProcess)
    {
        ...
        // if(blocksToProcess <= 2048) {
        //     Duplicate( inU32, int32_t(nounce1), mask_1, blocksToProcess / 16, dstBlockStride, RepeatStride);
        //     Duplicate( inU32, int32_t(nounce2), mask_2, blocksToProcess / 16, dstBlockStride, RepeatStride);
        //     Duplicate( inU32, int32_t(nounce3), mask_3, blocksToProcess / 16, dstBlockStride, RepeatStride);
        //     CreateVecIndex(inU32, int32_t(baseCounter * 4 - 3), mask_all, blocksToProcess / 16 , dstBlockStride ,RepeatStride);
        // }
        // else {
        //     #pragma unroll
        //     for(int32_t b = 0; b < 2  ; ++b) {
        //         int32_t offset = blocksToProcess * 2;
        //         Duplicate( inU32[b * offset], int32_t(nounce1), mask_1, blocksToProcess / 32, dstBlockStride, RepeatStride);
        //         Duplicate( inU32[b * offset], int32_t(nounce2), mask_2, blocksToProcess / 32, dstBlockStride, RepeatStride);
        //         Duplicate( inU32[b * offset], int32_t(nounce3), mask_3, blocksToProcess / 32, dstBlockStride, RepeatStride);
        //     }
        //     CreateVecIndex(inU32, int32_t(baseCounter * 4 - 3), mask_all, blocksToProcess / 16 - 1  , dstBlockStride ,RepeatStride);

        //     //blocksToProcess = 4096,repeattimes最大只能255，单独处理最后16个块s
        ...
        //         state0(lane) = inU32(Base + 0);
        //         state1(lane) = inU32(Base + 1);
        //         state2(lane) = inU32(Base + 2);
        //         state3(lane) = inU32(Base + 3);
        //     }
        //    AES128_Encrypt_Vector(outLocal, state0, state1, state2, state3, rkWordsLT, b);
        // }
                                                 ...
                                                 LocalTensor<uint32_t> rkWordsLT)
    {
        ...
        // AscendC::Scatter(outU32, xlocal0_u32, scatterIdx0U32, (uint32_t)0, VEC_BLOCKS);
        // AscendC::Scatter(outU32, xlocal1_u32, scatterIdx1U32, (uint32_t)0, VEC_BLOCKS);
        // AscendC::Scatter(outU32, xlocal2_u32, scatterIdx2U32, (uint32_t)0, VEC_BLOCKS);
        // AscendC::Scatter(outU32, xlocal3_u32, scatterIdx3U32, (uint32_t)0, VEC_BLOCKS);
    ...
    )
    {
        ...
        uint32_t t2 = t32[2];
        uint32_t t3 = t32[3];
// Middle 9 rounds via T-tables    AES128_NR
#pragma unroll
        for (int r = 1; r < AES128_NR; ++r)
        {
...
extern "C" __global__ __aicore__ void aes_vec_generate(
...
{
    aes_vec_generate<<<blockDim, nullptr, stream>>>(
